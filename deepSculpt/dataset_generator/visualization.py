...
class Visualizer:
            ...
            for i, angle in enumerate(angles):
                if i < len(axes):
                    ...
                    if materials is not None:
                        ...
                        log_info(
                            f"Plotted view {i} (rotation {angle*90}°) with materials"
                        )
                    else:
                        axes[i].voxels(
                            rotated_volumes,
                        ...
                        log_info(
                            f"Plotted view {i} (rotation {angle*90}°) without materials"
                        )
    ...
    @staticmethod
    def voxel_to_pointcloud(volumes: np.ndarray, subdivision: int = 3) -> np.ndarray:
        """
...
if __name__ == "__main__":
    ...
    visualizer.plot_single_view(
        void, color_void, angle=1, hide_axis=True, title="Single View (90°)", show=True
    )
