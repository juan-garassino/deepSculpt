...
import time
from deepSculpt.params import load_data,n_samples,void_dim,BUFFER_SIZE,BATCH_SIZE,EPOCHS,noise_dim,num_examples_to_generate,seed,BUCKET_NAME,BUCKET_TRAIN_DATA_PATH,MODEL_BASE_PATH
from deepSculpt.preprocessing import OneHotEncoderDecoder
from deepSculpt.data import DataLoaderCreator
from deepSculpt.model import make_three_dimentional_generator, make_three_dimentional_critic

data = DataLoaderCreator(path_volumes="raw-data[2022-07-26].npy", path_colors="color-raw-data[2022-07-26].npy")
...
train_dataset = Dataset.from_tensor_slices(o_encode).shuffle(BUFFER_SIZE).batch(BATCH_SIZE)
...
def train_step(images): # train for just ONE STEP aka one forward and back propagation
    ...
    noise = normal([BATCH_SIZE, noise_dim]) #tf.random.normal([BATCH_SIZE, noise_dim]) # generate the noises [batch size, latent space 100 dimention vector]
