...
print("\n⏹ " + Fore.YELLOW +
      "Just preproccess data from shape {} to {}".format(
          colors.shape, o_encode.shape) + Style.RESET_ALL)

print("\n⏹ " + Fore.YELLOW + "The classes are: {}".format(o_classes) +
      Style.RESET_ALL)

train_dataset = (
    Dataset.from_tensor_slices(o_encode).shuffle(BUFFER_SIZE).take(
        int(os.environ.get("TRAIN_SIZE"))).batch(
    ...
    for epoch in range(epochs):
        ...
        for index, image_batch in enumerate(dataset):
            ...
            if (index + 1) % MINIBATCHES[index]:
                ...
                print("\n⏩ " + Fore.MAGENTA + "Minibatch number %d" %
                      (index + 1, ) + Style.RESET_ALL + "\n")
