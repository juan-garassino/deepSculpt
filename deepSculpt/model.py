...
def make_three_dimentional_generator():
    ...
    model.add(layers.Dense(3*3*3*512, use_bias=False, input_shape=(noise_dim,))) # shape 512 noise vector, 7*7*256 flat layer to reshape [7,7,256] | 7 width 7 height 256 channels
    ...
    model.add(layers.Conv3DTranspose(512, (3, 3, 3), strides=(1, 1, 1), padding='same', use_bias=False, activation='relu')) # 128 Filters... to be the number of channels of the output, (5,5) kernel
    assert model.output_shape == (None, 3, 3, 3, 512)
    model.add(layers.BatchNormalization())
    model.add(layers.ReLU())

    model.add(layers.Conv3DTranspose(256, (3, 3, 3), strides=(2, 2, 2), padding='same', use_bias=False, activation='relu')) # 128 Filters... to be the number of channels of the output, (5,5) kernel
    assert model.output_shape == (None, 6, 6, 6, 256)
    model.add(layers.BatchNormalization())
    model.add(layers.ReLU())

    model.add(layers.Conv3DTranspose(128, (3, 3, 3), strides=(2, 2, 2), padding='same', use_bias=False, activation='relu')) # 128 Filters... to be the number of channels of the output, (5,5) kernel
    assert model.output_shape == (None, 12, 12, 12, 128)
    model.add(layers.BatchNormalization())
    model.add(layers.ReLU())

    model.add(layers.Conv3DTranspose(6, (3, 3, 3), strides=(2, 2, 2), padding='same', use_bias=False, activation='softmax'))
...
def make_three_dimentional_critic():
    model = Sequential()
    model.add(layers.Conv3D(64, (3, 3, 3), strides=(2, 2, 2), padding='same', input_shape=[void_dim, void_dim, void_dim, 6, 1]))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))

    model.add(layers.Conv3D(128, (3, 3, 3), strides=(2, 2, 2), padding='same', activation='relu'))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))

    model.add(layers.Conv3D(256, (3, 3, 3), strides=(2, 2, 2), padding='same', activation='relu'))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))

    model.add(layers.Conv3D(512, (3, 3, 3), strides=(2, 2, 2), padding='same', activation='relu'))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))
