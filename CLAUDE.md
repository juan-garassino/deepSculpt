...
- **Walk beauty pass**: `python scripts/render_walk.py <walk_volumes.pt|walk_epoch_NNN.pt> [--orbit 360 --fps 12 --color storm --mp4]` — marching-cubes + depth-shaded trisurf GIF (ax.voxels is unusably slow at 64³); iterate aesthetics from saved volumes without re-running the generator. Juan's preferred showcase walk (2026-07-12): 8 anchor seeds × `--steps 24` slerp via MODE=render, then `--orbit 360` (or 720) `--fps 12`, no boomerang; the ~35 min cloud HQ pass is the long pole — cheaper to re-render locally from the synced `walk_volumes.pt` (deepSculpt pyenv env has the deps).
